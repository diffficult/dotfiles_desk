...
// command assistant). The single "result" row is synthetic and the
// preview pane shows the tldr markdown for the first word (the tool
// name). This mode is inline-only: the right-hand preview is the UX,
// not a separate spawned terminal.
Item {
    id: tldrSearch
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // Synthetic single-row result. The preview pane owns the
            // experience; Enter does not need to launch a separate
            // terminal for tldr mode.
            tldrSearch.items = [{
            ...
            // tealdeer uses `--raw` / `-r` for markdown output; `-m`
            // is not supported here. 2>&1 folds the "documentation is
            // not available" message into the preview body. Positional
            // $1 keeps the tool name argv-safe.
